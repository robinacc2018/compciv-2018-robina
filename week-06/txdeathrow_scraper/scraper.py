...
def get_and_parse_inmate_rows():
    ...
    inmates = soup.select('tr')

    print (type(inmates[0]))
    return inmates[1:]
...
def wrangle_inmate_data_from_tag(rowtag):
    ...
    inmates = rowtag.select('td')
    inmate_data = {}
    inmate_data["tdcj_id"] = inmates[0].text.strip()

    url_short = inmates[1].find('a', href = True)['href']
    url = make_absolute_url(url_short)
    inmate_data['url'] = url

    inmate_data['last_name'] = inmates[2].text.strip()

    inmate_data['first_name'] = inmates[3].text.strip()

    inmate_data['birthdate'] = txdate_to_iso(inmates[4].text.strip())

    inmate_data['gender'] = inmates[5].text.strip()

    inmate_data['race'] = inmates[6].text.strip()

    inmate_data['date_received'] = txdate_to_iso(inmates[7].text.strip())

    inmate_data['county'] = inmates[8].text.strip()
